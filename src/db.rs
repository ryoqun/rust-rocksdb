...
impl<T: ThreadMode> DBWithThreadMode<T> {
    ...
    pub fn multi_get_cf<'a, 'b: 'a, K, I, W: 'b>(&'a self, keys: I) -> Result<Vec<Vec<u8>>, Error>
    where
        K: AsRef<[u8]>,
    ...
    fn drop_column_family(&self, cf_inner: *mut crate::ffi::rocksdb_column_family_handle_t) -> Result<(), Error> {
        unsafe {
        ...
        // family handle we have ever created by drop()-ing `cf` from this block
    }
}
...
impl DBWithThreadMode<SingleThreaded> {
    ...
    pub fn drop_cf(&mut self, name: &str) -> Result<(), Error> {
        if let Some(cf) = self.cfs.cfs.remove(name) {
            self.drop_column_family(cf.inner)
        } else {
...
impl DBWithThreadMode<MultiThreaded> {
    ...
    pub fn cf_handle(&self, name: &str) -> Option<Arc<BoundColumnFamily>> {
        let cf_ref = self.cfs.cfs.read().unwrap().get(name).cloned();
        // SAFETY: the new BoundColumnFamily here just adding lifetime...
        // so, same column family handle won't be destroyed twice...
        unsafe { std::mem::transmute(cf_ref) }
    }
}
